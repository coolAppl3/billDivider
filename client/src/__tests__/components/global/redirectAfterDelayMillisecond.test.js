...
describe(`redirectAfterDelayMillisecond(
  ...
  it('should always return undefined', () => {
    ...
    expect(redirectAfterDelayMillisecond('index.html', 2000, 'some message', 'danger')).toBeUndefined();
  });
  ...
  describe('tests focused on the target argument', () => {
    it(`should call messagePopup('Something went wrong', 'danger') and redirect the user to index.html if the target argument is falsy`, () => {
      ...
      expect(messagePopup).toHaveBeenCalledWith('Something went wrong.', 'danger', 2000);
    ...
    it(`should call messagePopup with the passed in arguments, and redirect the user accordingly, if the target argument is valid`, () => {
      jest.useFakeTimers();
      redirectAfterDelayMillisecond('redirect.html', 2000, 'Some message.', 'cta');
      jest.advanceTimersByTime(2000);
  
      expect(messagePopup).toHaveBeenCalledWith('Some message.', 'cta', 2000);
      expect(window.location.href).toBe('redirect.html');
    });
  })
  
  describe('tests focused on the delay argument', () => {
    it(`should reassign the delay argument to 2000 and continue normally, if the delay passed in is equal 0`, () => {
      jest.useFakeTimers();
      ...
      jest.advanceTimersByTime(1000); // marks 2000 milliseconds since the function was called
      expect(messagePopup).toHaveBeenCalledWith('Something went wrong.', 'danger', 2000);
      expect(window.location.href).toBe('redirect.html');
    });
  
    it(`should reassign the delay argument to 2000 and continue normally, if the delay passed in is less than 0`, () => {
      jest.useFakeTimers();
  
      redirectAfterDelayMillisecond('redirect.html', -1000);
      expect(window.location.href).not.toBe('redirect.html');
    ...
    it(`should reassign the delay argument to 2000 and continue normally, if the delay passed in is not a type of number`, () => {
      jest.useFakeTimers();
